...
create_pr_for_template() {
    ...
    file_status=$(git status $template_path/template.json --short | cut -d' ' -f 1)
    case $file_status in
        A)
            label_arg="--label new"
            ;;
    ...
    git commit -m "(auto) New template $template"
    git push origin $branch

    gh pr create -d $label_arg \
        --title "(auto) Publish template: $template." \
